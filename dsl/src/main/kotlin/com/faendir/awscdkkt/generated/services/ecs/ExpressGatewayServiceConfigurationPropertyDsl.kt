package com.faendir.awscdkkt.generated.services.ecs

import com.faendir.awscdkkt.AwsCdkDsl
import javax.`annotation`.Generated
import kotlin.Unit
import software.amazon.awscdk.services.ecs.CfnExpressGatewayService

@Generated
public fun buildExpressGatewayServiceConfigurationProperty(initializer: @AwsCdkDsl CfnExpressGatewayService.ExpressGatewayServiceConfigurationProperty.Builder.() -> Unit = {}): CfnExpressGatewayService.ExpressGatewayServiceConfigurationProperty = CfnExpressGatewayService.ExpressGatewayServiceConfigurationProperty.Builder().apply(initializer).build()
