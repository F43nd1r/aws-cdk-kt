package com.faendir.awscdkkt.generated.services.quicksight

import com.faendir.awscdkkt.AwsCdkDsl
import javax.`annotation`.Generated
import kotlin.Unit
import software.amazon.awscdk.services.quicksight.CfnAnalysis

@Generated
public fun buildDateTimeValueWhenUnsetConfigurationProperty(initializer: @AwsCdkDsl
    CfnAnalysis.DateTimeValueWhenUnsetConfigurationProperty.Builder.() -> Unit):
    CfnAnalysis.DateTimeValueWhenUnsetConfigurationProperty =
    CfnAnalysis.DateTimeValueWhenUnsetConfigurationProperty.Builder().apply(initializer).build()
