package com.faendir.awscdkkt.generated.services.glue

import com.faendir.awscdkkt.AwsCdkDsl
import javax.`annotation`.Generated
import kotlin.Unit
import software.amazon.awscdk.services.glue.CfnIntegrationResourceProperty

@Generated
public fun buildTargetProcessingPropertiesProperty(initializer: @AwsCdkDsl CfnIntegrationResourceProperty.TargetProcessingPropertiesProperty.Builder.() -> Unit = {}): CfnIntegrationResourceProperty.TargetProcessingPropertiesProperty = CfnIntegrationResourceProperty.TargetProcessingPropertiesProperty.Builder().apply(initializer).build()
