package com.faendir.awscdkkt.generated.services.quicksight

import com.faendir.awscdkkt.AwsCdkDsl
import javax.`annotation`.Generated
import kotlin.Unit
import software.amazon.awscdk.services.quicksight.CfnAnalysis

@Generated
public fun buildConditionalFormattingCustomIconOptionsProperty(initializer: @AwsCdkDsl
    CfnAnalysis.ConditionalFormattingCustomIconOptionsProperty.Builder.() -> Unit):
    CfnAnalysis.ConditionalFormattingCustomIconOptionsProperty =
    CfnAnalysis.ConditionalFormattingCustomIconOptionsProperty.Builder().apply(initializer).build()
