package com.faendir.awscdkkt.generated.services.quicksight

import com.faendir.awscdkkt.AwsCdkDsl
import javax.`annotation`.Generated
import kotlin.Unit
import software.amazon.awscdk.services.quicksight.CfnDashboard

@Generated
public fun buildWaterfallChartAggregatedFieldWellsProperty(initializer: @AwsCdkDsl
    CfnDashboard.WaterfallChartAggregatedFieldWellsProperty.Builder.() -> Unit):
    CfnDashboard.WaterfallChartAggregatedFieldWellsProperty =
    CfnDashboard.WaterfallChartAggregatedFieldWellsProperty.Builder().apply(initializer).build()
